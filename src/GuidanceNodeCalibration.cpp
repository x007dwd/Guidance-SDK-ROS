...
// A nice TODO will be to wrap the driver using the ROS standard
// cam_info_manager.
// Hackish code to read cam params from YAML
// adapted from cam_info_manager and camera_calibration_parser
// https://github.com/ros-perception/image_common/blob/hydro-devel/camera_calibration_parsers/src/parse_yml.cpp
...
void transfer_SimpleMatrix_from_YML_to_ROSmsg(const YAML::Node &node,
                                              SimpleMatrix &m) {
  int rows, cols;
...
void read_params_from_yaml_and_fill_cam_info_msg(
    std::string &file_name, sensor_msgs::CameraInfo &cam_info) {
...
int my_callback(int data_type, int data_len, char *content) {
  ...
  if (e_image == data_type && NULL != content) {
    ...
    if (data->m_greyscale_image_left[CAMERA_ID]) {
      memcpy(g_greyscale_image_left.data,
             data->m_greyscale_image_left[CAMERA_ID], IMAGE_SIZE);
      ...
      try {
        read_params_from_yaml_and_fill_cam_info_msg(camera_params_left,
                                                    g_cam_info_left);
    ...
    if (data->m_greyscale_image_right[CAMERA_ID]) {
      memcpy(g_greyscale_image_right.data,
             data->m_greyscale_image_right[CAMERA_ID], IMAGE_SIZE);
      ...
      try {
        read_params_from_yaml_and_fill_cam_info_msg(camera_params_right,
                                                    g_cam_info_right);
...
int main(int argc, char **argv) {
  ...
  left_image_pub =
      my_node.advertise<sensor_msgs::Image>("/guidance/left/image_raw", 1);
  right_image_pub =
      my_node.advertise<sensor_msgs::Image>("/guidance/right/image_raw", 1);
  imu_pub =
      my_node.advertise<geometry_msgs::TransformStamped>("/guidance/imu", 1);
  velocity_pub =
      my_node.advertise<geometry_msgs::Vector3Stamped>("/guidance/velocity", 1);
  obstacle_distance_pub = my_node.advertise<sensor_msgs::LaserScan>(
      "/guidance/obstacle_distance", 1);
  ultrasonic_pub =
      my_node.advertise<sensor_msgs::LaserScan>("/guidance/ultrasonic", 1);
  cam_info_right_pub = my_node.advertise<sensor_msgs::CameraInfo>(
      "/guidance/right/camera_info", 1);
  cam_info_left_pub = my_node.advertise<sensor_msgs::CameraInfo>(
      "/guidance/left/camera_info", 1);
  ...
  for (int i = 0; i < CAMERA_PAIR_NUM; i++) {
    std::cout << cali[i].cu << "\t" << cali[i].cv << "\t" << cali[i].focal
              << "\t" << cali[i].baseline << std::endl;
  }
  ...
  while (ros::ok()) {
    ...
    if (key > 0)
      ...
      if (key == 'j' || key == 'k' || key == 'm' || key == 'n') {
        ...
        std::cout << "Setting exposure parameters....SensorId=" << CAMERA_ID
                  << std::endl;
        para.m_camera_pair_index = CAMERA_ID;
