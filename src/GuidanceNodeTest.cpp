...
void right_image_callback(const sensor_msgs::ImageConstPtr &right_img) {
  ...
  try {
    cv_ptr =
        cv_bridge::toCvCopy(right_img, sensor_msgs::image_encodings::MONO8);
  } catch (cv_bridge::Exception &e) {
    ROS_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
...
void depth_image_callback(const sensor_msgs::ImageConstPtr &depth_img) {
  ...
  try {
    cv_ptr =
        cv_bridge::toCvCopy(depth_img, sensor_msgs::image_encodings::MONO16);
  } catch (cv_bridge::Exception &e) {
    ROS_ERROR("cv_bridge exception: %s", e.what());
    return;
  }
...
void imu_callback(const geometry_msgs::TransformStamped &g_imu) {
  ...
  printf("imu: [%f %f %f %f %f %f %f]\n", g_imu.transform.translation.x,
         g_imu.transform.translation.y, g_imu.transform.translation.z,
         g_imu.transform.rotation.x, g_imu.transform.rotation.y,
         g_imu.transform.rotation.z, g_imu.transform.rotation.w);
  geometry_msgs::PoseStamped pose_i;
  pose_i.header = g_imu.header;
  pose_i.pose.position.x = g_imu.transform.translation.x;
  pose_i.pose.position.x = g_imu.transform.translation.y;
  pose_i.pose.position.x = g_imu.transform.translation.z;
  pose_i.pose.orientation = g_imu.transform.rotation;
  imu_pub.publish(pose_i);
}
...
int main(int argc, char **argv) {
  ...
  left_image_sub =
      my_node.subscribe("/guidance/left_image", 10, left_image_callback);
  right_image_sub =
      my_node.subscribe("/guidance/right_image", 10, right_image_callback);
  depth_image_sub =
      my_node.subscribe("/guidance/depth_image", 10, depth_image_callback);
  ...
  obstacle_distance_sub = my_node.subscribe("/guidance/obstacle_distance", 1,
  ...
  imu_pub =
      my_node.advertise<geometry_msgs::PoseStamped>("guidance/imu_pose", 1);
  while (ros::ok())
    ros::spinOnce();
